...
class ClimateSubmodel:
    ...
    def run(self, t, year, forc, gross_output):
        ...
        def calculate_total_SLR(t, year):
            ...
            self.THERMEQUIL = self.temp_atm[t] * thermeq
            self.SLRTHERM[t] = self.SLRTHERM[t-1] + thermadj * (self.THERMEQUIL - self.SLRTHERM[t-1])
            ...
            self.GSICREMAIN = gsictotal - self.GSICCUM[t-1]
            self.GSICMELTRATE = (gsicmelt * self.delta_t * (self.GSICREMAIN / gsictotal) ** gsicexp * self.temp_atm[t])
            self.GSICCUM[t] = self.GSICCUM[t-1] + self.GSICMELTRATE
            ...
            if year <= 2115:
                if self.temp_atm[t] < 3:
                    self.AISMELTRATE = (aismeltlow * self.temp_atm[t] * aisratio + aisintercept)
                else:
                    self.AISMELTRATE = (aisinflection * aismeltlow + aismeltup * (self.temp_atm[t] - 3.0) + aisintercept)
            else:
                if self.temp_atm[t] < 3:
                    self.AISMELTRATE = (aismeltlow * self.temp_atm[t] * aisratio + aismelt0)
                else:
                    self.AISMELTRATE = (aisinflection * aismeltlow + aismeltup * (self.temp_atm[t] - 3) + aismelt0)

            self.AISCUM[t] = self.AISCUM[t-1] + self.AISMELTRATE / 100

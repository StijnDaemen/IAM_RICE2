...
class WelfareSubmodel:
    def __init__(self, years, regions):
        ...
        self.Utility_Input = pd.read_excel("RICE_model/input_data/RICE_input_data.xlsx", sheet_name="Stijn_RICE_input", usecols=range(0, 36), nrows=12)
        ...
        self.temp_threshold = 1.5  # 1.5 Degrees Celsius, in accordance with the IPCC and COP21 Paris.

    def run_utilitarian(self, t, year, CPC, labour_force, damages, net_output, temp_atm):
        # irstp: Initial rate of social time preference per year
        self.util_sdr[:, t] = 1 / ((1 + self.irstp) ** (self.delta_t * (t)))
        ...
        np.where(self.temp_overshoots >= self.temp_overshoots_lower_bound, self.temp_overshoots, self.temp_overshoots_lower_bound)
        return

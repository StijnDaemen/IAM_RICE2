...
# ToDO
# test if dictionaries are faster than np arrays
...
class EconomicSubmodel:
    def __init__(self, years, regions):
        ...
        self.input_data = pd.read_excel("RICE_model/input_data/RICE_input_data.xlsx", sheet_name="Stijn_RICE_input", usecols=range(0, 36), nrows=12)
        ...
        self.must_change_10yr_only_input_data_pop_growth = pd.read_excel("RICE_model/input_data/RICE_input_data.xlsx", sheet_name="Stijn_MUST_change_input", usecols=range(34, 65), nrows=12)
        self.income_shares = pd.read_excel("RICE_model/input_data/RICE_input_data.xlsx", sheet_name='Ivar_RICE_income_shares',usecols=range(1, 6)).to_numpy()
    ...
    def run_gross(self, t, year, mu_target, sr):
        ...
        def calculate_total_factor_productivity(t):
            # Below based on bad but verified 10yr only data tfp growth data
